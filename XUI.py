...
with col3:
    tabs = st.tabs(["Setting", "Statement", "Profile"])
...
if not st.session_state.user:
    ...
    with profile_col:
        if st.session_state.user:
            ...
            st.text(f"Name: {st.session_state.user['name']}")
            st.text(f"User ID: {st.session_state.user['user_id']}")
            st.text(f"Credit Score: {st.session_state.user['credit_score']}")
            st.text(f"Last Login: {st.session_state.user['last_login']}")

            # Show user's products and files
            st.subheader("Your Products")
            for product in st.session_state.user['products']:
                st.write(f"• {product}")
